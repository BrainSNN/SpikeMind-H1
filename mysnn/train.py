...
import pickle
from model import EEG_SNN, EEGNet, SEEDEEGDataset
...
def evaluate(model, loader, criterion, device):
    ...
    for x, y in loader:
        ...
        a = (prob.max(1).values>0.7).to(torch.float32)
        all_prob.extend(a.cpu().numpy().tolist() if prob.shape[1] == 2 else prob.cpu().numpy().tolist())
...
def main():
    ...
    val_path = '/home/data/data/NeuroLM_data/dataset/MODMA/val/'
    ...
    model = EEGNet(nb_classes=2,Channels=19, Samples=2000,dropout_rate=0.1).to(cfg.device)
    # model = EEG_SNN(chans=19,num_nodes=128).to(cfg.device)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        val_metrics = evaluate(model, train_loader, criterion, cfg.device)

        log = {
            "epoch": epoch,
            "train_loss": train_loss,
            "val_acc": val_metrics["acc"],
            "val_score mean": float(np.mean(val_metrics["score"])),

        }
        print(log)
    torch.save(model.state_dict(),'./checkpoints/EEGNet.pth')

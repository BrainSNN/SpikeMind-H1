...
from model.model_audio import AudioEncoder
import numpy as np
...
class NeuroLM(nn.Module):
    ...
    def __init__(self,
        ...
        self.AudioEncoder = AudioEncoder('/home/data/data/models/Qwen2-Audio-7B-Instruct',n_embd=n_embd) #这里换成自己的路径
        self.audio_size = audio_size
        self.split_embd = nn.Embedding(1,self.GPT2.config.n_embd)
    ...
    def forward(self, x_eeg=None, y_eeg=None, x_text=None, y_text=None, input_chans=None, input_time=None, input_mask=None, eeg_mask=None, eeg_text_mask=None,audio=None):
        """
        x_eeg: shape [B, N1, T]
        x_text: shape [B, N2]
        """
        ...
        if audio is not None:
            audio = self.AudioEncoder(audio)[:,:self.audio_size]
            split_embd = self.split_embd(torch.zeros(audio.shape[0]).to(torch.long).to(audio.device)).unsqueeze(1)
            x_eeg = torch.cat([x_eeg,split_embd,audio],dim=1).contiguous()
            audio_time = np.array([np.arange(int(input_time.max()),int(input_time.max())+self.audio_size+1) for _ in range(input_time.shape[0])])
            audio_time = torch.tensor(audio_time).to(torch.int)
            audio_time = audio_time.to(input_time.device)
            input_time = torch.cat([input_time,audio_time],dim=1)
            if eeg_mask is not None:
                audio_mask = np.array([np.ones(self.audio_size) for _ in range(input_time.shape[0])])
                audio_mask = torch.tensor(audio_mask).to(torch.bool)
                audio_mask = audio_mask.to(eeg_mask.device)
                eeg_mask = torch.cat([eeg_mask,audio_mask],dim=1)
    ...
    @torch.no_grad()
    def generate(self, x_eeg, x_text, input_chans, input_time, input_mask, eeg_mask=None, eeg_text_mask=None,audio=None, max_new_tokens=10, temperature=1.0, top_k=1):
        if x_eeg is not None:
            input_mask = input_mask.unsqueeze(1).repeat(1, x_eeg.size(1), 1).unsqueeze(1)
            x_eeg = self.tokenizer(x_eeg, input_chans, input_time, input_mask, return_all_tokens=True)
            x_eeg = self.encode_transform_layer(x_eeg)

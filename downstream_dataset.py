...
# class MODMALoader(Dataset):
#     # abnormal: 1
#     # normal: 0
#     def __init__(self, root, files, sampling_rate=200, eeg_max_len=-1, text_max_len=-1,audio_max_len=-1, is_instruct=False, is_val=False):
#         self.root = root
#         self.files = files
#         self.default_rate = 200
#         self.sampling_rate = sampling_rate
...
#         self.ch_names = ['FP1', 'FP2', 'F7', 'F3', 'F4', 'F8', 'C3', 'C4', 'T3', 'T4', 'P3', 'P4', 'PZ', 'FZ', 'T5', 'T6', 'O1', 'OZ', 'O2']
...
#             # 50257 for [SEP]
#             self.text = {
#                 1: torch.IntTensor([50257] + encode('Question: Does this EEG segment get MDD? Answer: Yes <|endoftext|>')),
#                 0: torch.IntTensor([50257] + encode('Question: Does this EEG segment get MDD? Answer: No <|endoftext|>'))
#             }
#             self.prompt = torch.IntTensor([50257] + encode('Question: Is this EEG segment abnormal? Answer:'))
...
#         time = data.size(1) // 200
#         input_time = [i  for i in range(time) for _ in range(data.size(0))]
#         # audio_time = [i for i in range(self.audio_max_len)]
#         # audio_time = torch.IntTensor(audio_time)
#         data = rearrange(data, 'N (A T) -> (A N) T', T=200)
...
#         num_tokens = X_eeg.size(0)+101 + text.size(0)
#         gpt_mask = torch.tril(torch.ones(num_tokens, num_tokens)).view(1, num_tokens, num_tokens)
#         num_chans = len(ch_names)
#         for i in range(time):
#             gpt_mask[:, i * num_chans:(i + 1) * num_chans,  i * num_chans:(i + 1) * num_chans] = 1
...
#         if self.is_val:
#             return X_eeg, text, Y, input_chans, input_time, eeg_mask.bool(), gpt_mask.bool(),audio
        
#         Y_text = torch.full_like(text, fill_value=-1)
#         prompt_len = self.prompt.size(0)
#         Y_text[prompt_len - 1:valid_text_len - 1] = text[prompt_len:valid_text_len]
#         return X_eeg, text, Y_text, input_chans, input_time, eeg_mask.bool(), gpt_mask.bool(),audio
...
class MODMALoader(Dataset):
    ...
    def __init__(self, root, files, sampling_rate=200, eeg_max_len=-1, text_max_len=-1,audio_max_len=-1, is_instruct=False, is_val=False):
        ...
        if is_instruct:
            ...
            # 50257 for [SEP]
            self.text = {
                1: torch.IntTensor([50257] + encode('Question: Does this EEG and audio get MDD? Answer: Yes <|endoftext|>')),
                0: torch.IntTensor([50257] + encode('Question: Does this EEG and audio get MDD? Answer: No <|endoftext|>'))
            }
            self.prompt = torch.IntTensor([50257] + encode('Question: Does this EEG and audio get MDD? Answer: '))
    ...
    def __getitem__(self, index):
        ...
        num_tokens = X_eeg.size(0)+201 + text.size(0)
        gpt_mask = torch.tril(torch.ones(num_tokens, num_tokens)).view(1, num_tokens, num_tokens)
        num_chans = len(ch_names)
        for i in range(time):
            gpt_mask[:, i * num_chans:(i + 1) * num_chans,  i * num_chans:(i + 1) * num_chans] = 1
...
class OpenslrLoader(Dataset):
    ...
    def __init__(self, root, files, sampling_rate=200, eeg_max_len=-1, text_max_len=-1,audio_max_len=-1, is_instruct=False, is_val=False):
        ...
        if is_instruct:
            ...
            #     0: torch.IntTensor([50257] + encode('Question: Does this EEG segment get MDD? Answer: No <|endoftext|>'))
            # }
            self.prompt = torch.IntTensor([50257] + self.encode("Convert this audio into Chinese: "))
    ...
    def __getitem__(self, index):
        ...
        if self.is_val:
            ...
            text = torch.cat([self.prompt,torch.IntTensor(self.encode(Y+"<|endoftext|>"))])
            # pad text to text_max_len
            valid_text_len = text.size(0)
            if self.text_max_len > valid_text_len:
                text_pad = torch.full((self.text_max_len,), fill_value=50256)
...
#             # 50257 for [SEP]
#             self.text = {
#                 1: torch.IntTensor([50257] + encode('Con: Yes <|endoftext|>')),
#                 0: torch.IntTensor([50257] + encode('Question: Does this EEG segment get MDD? Answer: No <|endoftext|>'))
#             }
#             self.prompt = torch.IntTensor([50257]+self.encode("Convert this audio into Chinese: "))
...
#             text = self.prompt
#         else:
#             text = torch.IntTensor([50257]+self.encode("Convert this audio into Chinese: "+text))
#             # pad text to text_max_len
#             valid_text_len = text.size(0)
#             if self.text_max_len > valid_text_len:
#                 text_pad = torch.full((self.text_max_len,), fill_value=50256)
...
#         if self.is_val:
#             return text, Y, input_chans, input_time, eeg_mask.bool(), gpt_mask.bool(),audio
        
#         Y_text = torch.full_like(text, fill_value=-1)
#         prompt_len = self.prompt.size(0)
#         Y_text[prompt_len - 1:valid_text_len - 1] = text[prompt_len:valid_text_len]
#         return text, Y_text, input_chans, input_time, eeg_mask.bool(), gpt_mask.bool(),audio

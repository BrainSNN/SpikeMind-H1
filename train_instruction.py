...
import tiktoken
from utils import prepare_TUAB_dataset, prepare_TUEV_dataset, prepare_TUSL_dataset, prepare_HMC_dataset, prepare_Workload_dataset,prepare_MODMA_dataset, cosine_scheduler, get_metrics
...
def main(args):
    ...
    if init_from == 'resume':
        ...
        start_epoch = 0
    torch.save(model.tokenizer,'./checkpoints/neuraltransformer.pt')
...
def evaluate(model, dataset_info, dataloader, decode):
    ...
    for _, (batch) in enumerate(dataloader):
        ...
        with ctx:
            text = model.generate(X_eeg, X_text, input_chans, input_time, input_mask, eeg_text_mask=gpt_mask,audio=audio, max_new_tokens=5)
            text = text[:, 1:] # remove [SEP] token
            for i, t in enumerate(text):
                pred_string = decode(t.tolist())

                pred,a = get_pred(pred_string, dataset_info)
                texts.append(a)

...
import tiktoken
from utils import prepare_TUAB_dataset, prepare_TUEV_dataset, prepare_TUSL_dataset, prepare_HMC_dataset, prepare_Workload_dataset,prepare_MODMA_dataset, cosine_scheduler, get_metrics,prepare_Openslr_dataset
...
def get_instruct_datasets(args, downstream_dataset: str, eeg_max_len=-1, text_max_len=-1):
        ...
        elif downstream_dataset == 'MODMA':
            dataset_train, dataset_test, dataset_val = prepare_MODMA_dataset(Path(args.dataset_dir, 'MODMA'), is_instruct=True, 
                                                                            eeg_max_len=eeg_max_len, text_max_len=text_max_len)

            dataset_info['metrics'] = ["pr_auc", "roc_auc", "accuracy", "balanced_accuracy"]
            dataset_info['is_binary'] = True
            dataset_info['result_idx'] = 8
            dataset_info['label_dic'] = {'Yes': 1, 'No': 0}
        elif downstream_dataset == 'Openslr':
            dataset_train, dataset_test, dataset_val = prepare_Openslr_dataset(Path(args.dataset_dir, 'openslr'), is_instruct=True, 
                                                                            eeg_max_len=eeg_max_len, text_max_len=text_max_len)

            dataset_info['metrics'] = ["BLEU"]
            dataset_info['is_binary'] = True
            dataset_info['result_idx'] = 8
            dataset_info['label_dic'] = {'Yes': 1, 'No': 0}
...
def main(args):
    ...
    all_datasets = []
    # for name in ['TUAB','HMC', 'Workload',]:
    for name in ['Openslr']:
    # for name in ['TUAB', 'TUEV', 'SEED', 'HMC', 'Workload', 'TUSL']:
        all_datasets.append(get_instruct_datasets(args, name, eeg_max_len=276, text_max_len=200))
    if concat_datasets:
    ...
    if init_from == 'resume':
        ...
        # 执行：export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
        checkpoint = torch.load(ckpt_path, map_location=device,weights_only=False)
        checkpoint_model_args = checkpoint['model_args']
        # force these config attributes to be equal otherwise we can't even resume training
        # the rest of the attributes (e.g. dropout) can stay as desired from command line
    ...
    # optimizer = torch.optim.Adam(model.AudioEncoder.parameters(),lr=args.learning_rate,weight_decay=args.weight_decay)
    # compile the model
    if compile:
    ...
    for epoch in range(start_epoch, args.epochs):
        for dataset_info in datasets:
            ...
            for step, (batch) in enumerate(dataset_info['data_loader_train']):
                ...
                try:
                    X_eeg, X_text, Y_text, input_chans, input_time, input_mask, gpt_mask,audio = batch
                except:
                ...
                Y_eeg = torch.full((X_eeg.size(0), X_eeg.size(1)+201), fill_value=-1-raw_model.GPT2.config.vocab_size).to(device, non_blocking=True)
                ...
                with ctx:
                    loss1, log1, logits = model(X_eeg, Y_eeg, X_text, Y_text, input_chans, input_time, input_mask, eeg_text_mask=gpt_mask,audio=audio)
...
def evaluate(model, dataset_info, dataloader, decode):
    ...
    for _, (batch) in enumerate(dataloader):
        X_eeg, X_text, label, input_chans, input_time, input_mask, gpt_mask,audio = batch
        X_eeg = X_eeg.float().to(device, non_blocking=True)
        X_text = X_text.to(device, non_blocking=True)
        input_chans = input_chans.to(device, non_blocking=True)
        input_time = input_time.to(device, non_blocking=True)
        ...
        with ctx:
            text = model.generate(X_eeg, X_text, input_chans, input_time, input_mask, eeg_text_mask=gpt_mask,audio=audio, max_new_tokens=50)
            text = text[:, 8:] # remove [SEP] token
            text = [decode(i.cpu().numpy().tolist()) for i in text]
            bleu.append(utils.calculate_batch_bleu_nltk(text,label))
...
def get_args():
    ...
    parser.add_argument('--dataset_dir', default='/home/data/data/NeuroLM_data/dataset', help='path where to save, empty for no saving')
    ...
    parser.add_argument('--wandb_project', default='SpikeMind')
    parser.add_argument('--wandb_runname', default='instruction-B')
    parser.add_argument('--wandb_api_key',default='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', type=str)
    # training args
